...
# Polygon to represent an obstacle
class Poly:
...
# Utility function to extract obstacles as polygons from map data
...
# Class responsible for 3D route planning  
# Searches route path. Works with NED coordinates (Local Position) only.
class RoutePlanner():
...
#                     print("Can NOT Connect")
    ...
    def get_flight_waypoints(self, start_ned, goal_ned, prune_path=True):
        ...
        # IF Graph_Goal_Node ==> Map_Goal_Location is not found, then we set goal to the Graph_Goal_Node. 
...
#             print("\nRoute Planner - Found Graph Path ({0} Waypoints): {1}".format(len(graph_path), graph_path))
